function calculate() {
  var teamODD = document.getElementById('odd').value;
  var bet = document.getElementById('valor').value - (document.getElementById('valor').value * 0.3) ;
  var result = document.getElementById('ganhos');
  var myResult = teamODD * bet;
   ...
   if ($("#money").length > 0) {
        ...
        $.ajax({
            ...
            success: function (result) {
                    ...
                    if (result[0] !== "admin"){
                    document.getElementById('terminarApostasForm').style.display='none'
                    document.getElementById('inserirApostaForm').style.display='none'
                    document.getElementById('betform').style.display=''
                    }
...
function bet()  {
    ...
    $.ajax({
        ...
        success: function (result) {
           ...
           }else if (result["result"] == 1 && result["type"] == "bet" && result["text"] == "betNotregist"){
                 alert("Bet not registered");
                 console.log("Bet not registered");

            }else if (result["result"] == 1 && result["type"] == "bet" && result["text"] == "semsaldo"){
                 alert("Sem saldo");
...
function inserirAposta()  {
    var data = $('#inserirApostaForm').serialize();
    console.log(data);
...
function terminarApostas()  {
    ...
    $.ajax({
        type: 'POST',
        url: "http://127.0.0.1:4444/terminate",
        data: {'submit':true},
        headers: {'X-AUTH-TOKEN': window.location.hash.substring(1)},
        dataType: 'json',
        success: function (result) {
